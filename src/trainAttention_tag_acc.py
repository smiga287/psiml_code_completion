...
def train():
    ...
    train_split_idx = int(len(data_manager.get_data()) * 0.3)
    validate_split_idx = int(len(data_manager.get_data()) * 0.4)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        pogadjanja = [ [0, 0, i] for i in range(199)]
        ...
        with torch.no_grad():
            for i, (sentence, y) in tqdm(
                ...
                y_pred_val = model(sentence)
                y = y.to(device)
                y_predictions = y_pred_val.argmax(dim=1)
                correct_mask = y_predictions == y[:, 0]
                
                for x in y[:,0].long():
                    pogadjanja[x][1]+=1
                for x in y[correct_mask,0].long():
                    pogadjanja[x][0]+=1

                correct_val += (correct_mask).sum().item()
        ...
        pogadjanja.sort(key=cmp)
        pogadjanja = [ [ x, y, idx_to_tag[z] ] for x,y,z in pogadjanja ]
        print(pogadjanja)
